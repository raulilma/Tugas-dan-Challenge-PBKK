            ...
            <div class="row page-bg">
                <div class="p-4 col-md-12">
                    <div class="text-center top-icon">
                        ...
                        <form id="form-login" action="{{ route('produk.update', $data->id) }}" method="post" onsubmit="return confirm('Apakah Anda Yakin Edit Data ?');">
                            @csrf

                            <div>
                                <input class="mt-3 form-control form-control-lg @error('judul') is-invalid @enderror" name="judul" type="text"
                                       placeholder="Judul" value="{{ old('judul', $data->judul ? $data->judul : 'Tidak Ada Data') }}" autofocus required>
                            </div>
                            ...
                            <div>
                                <input class="mt-3 form-control form-control-lg @error('kategori') is-invalid @enderror" name="kategori" type="text"
                                       placeholder="Kategori" value="{{ $data->kategori ? $data->kategori : 'Tidak Ada Data' }}" autofocus required>
                            </div>
                            ...
                            <div>
                                <input class="mt-3 form-control form-control-lg @error('stock') is-invalid @enderror" name="stock" type="text"
                                       placeholder="Stock" value="{{ $data->stock ? $data->stock : 'Tidak Ada Data' }}" autofocus required>
                            </div>
                            ...
                            <div>
                                <input class="mt-3 form-control form-control-lg @error('penjual') is-invalid @enderror" name="penjual" type="text"
                                       placeholder="Penjual" value="{{ $data->penjual ? $data->penjual : 'Tidak Ada Data' }}" autofocus required>
                            </div>
